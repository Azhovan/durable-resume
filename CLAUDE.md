...
- **Wrap errors with `%w` and reuse exported sentinels.** New failure modes that callers/tests may branch on should use (or add to) the sentinels in `download.go`: `ErrNoURL`, `ErrUnsupportedScheme`, `ErrRemoteChanged`, `ErrSizeMismatch`, `ErrChecksumMismatch`, `ErrRangeNot206`, `ErrChunkFailed`, `ErrInvalidProxy`. Keep the `download:`/`verify:` message prefixes.
