...
func TestFetchChunkWritesAtOffset(t *testing.T) {
	...
	err := fetchChunk(context.Background(), srv.Client(), srv.URL, nil, ch, w, onBytes, nil, nil)
...
func TestFetchChunkRejectsNon206(t *testing.T) {
	...
	err := fetchChunk(context.Background(), srv.Client(), srv.URL, nil, ch, w, func(int64) {}, nil, nil)
	require.Error(t, err)
...
func TestFetchChunkNothingToDo(t *testing.T) {
	...
	called := false
	err := fetchChunk(context.Background(), http.DefaultClient, "http://invalid.invalid", nil, ch, w, func(int64) { called = true }, nil, nil)
	require.NoError(t, err)
